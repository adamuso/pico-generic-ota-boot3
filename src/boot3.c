...
void in_boot3_critical_section boot3_exit()
{
...
void in_boot3_critical_section boot3_copy_pending_to_current_and_exit()
{
    ...
    for (size_t i = 0; i < pending_state->prelude.program_size; i += sizeof(buffer)) {
        ...
        boot3_memcpy(pending_program + i, buffer, sizeof(buffer));
...
void in_boot3_section boot3_check_state()
{
    ...
    // - Its checksum is different from the current state's checksum (to distinguish between a pending that has been 
    ...
    if (
        ...
        if (
            ...
            boot3_copy_pending_to_current_and_exit();
            for (;;);
        }
    }
